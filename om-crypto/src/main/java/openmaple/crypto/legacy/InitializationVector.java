...
/**
 * A legacy protocol-specific initialization vector with built-in support for rolling.
 *
 * @author Aaron Weiss
 * @version 1.0.0
 * @since 8/19/13
...
public class InitializationVector {
	/**
	 * A reverse-engineered shuffling key for new initialization vectors.
	 */
	private static final byte[] shufflingKey = {
	...
	public void roll() {
		...
		for (byte b : this.iv) {
			iv[0] += shufflingKey[iv[1]] - b;
			iv[1] -= iv[2] ^ shufflingKey[b];
			iv[2] ^= shufflingKey[iv[3]] + b;
			iv[3] -= iv[0] - shufflingKey[b];
			int merged = (iv[0] & 0xFF) | ((iv[1] << 8) & 0xFF00) | ((iv[2] << 16) & 0xFF0000) | ((iv[3] << 24) & 0xFF000000);
			merged = (merged << 3) | (merged >>> 0x1D);
			iv[0] = (byte) (merged & 0xFF);
			iv[1] = (byte) ((merged >> 8) & 0xFF);
			iv[2] = (byte) ((merged >> 16) & 0xFF);
			iv[3] = (byte) ((merged >> 24) & 0xFF);
		}
		this.iv = iv;
	}
}
